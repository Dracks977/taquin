...
class GameManager extends React.Component {
	...
	checkmove(number, x, y) {
		...
		if (this.state.tab[x+1] && this.state.tab[x+1][y] && this.state.tab[x+1][y] === "X") {
			this.move({x: x, y:y, number: number}, {x: x+1, y:y})
		} else if (this.state.tab[x-1] && this.state.tab[x-1][y] && this.state.tab[x-1][y] === "X") {
			this.move({x: x, y:y, number: number}, {x: x-1, y:y})
	...
	render() {
		return (
			<Suspense fallback={<div>Chargement...</div>}>
				<p className={cx("win", {hidden: !this.state.win?true:false})} >GG you win =)</p>
        		<Grid grid={this.state.tab} onClick={this.checkmove} />
      		</Suspense>
			);
		}
	}
